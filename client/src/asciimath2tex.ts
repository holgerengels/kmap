...
export default class AsciiMathParser {
  ...
  private relations: { tex: string; asciimath: string }[];
  private constants: { tex: string; asciimath: string }[];
  private left_brackets: mapping[];
  private right_brackets: mapping[];
  private leftright_brackets: mapping[];
  private unary_symbols: mapping[];
  private binary_symbols: mapping[];
  ...
  matrix(pos = 0) {
    let left: {} | undefined = this.left_bracket(pos);
    let lr = false;
    if(!left) {
  ...
  matrix_contents(pos = 0, leftright = false) {
    let rows: {} = [];
    let end = pos;
    let row_length: number | undefined;
    let column_desc: string | undefined;
    let is_array = false;
